...
function get_table()
{
		...
		foreach ($sewa_list as $sewa) {
			if (!in_array($sewa['sk_sr'], $sk_names)) {
				...
				if (strcmp($sk[0], "SR") == 0) {
					...
					foreach ($query as $q) {
						array_push($sr_short_array, $q['short_name']);
					}
				}
			}
		}
	...
	sort($sr_short_array);
	foreach ($sr_short_array as $sr_short) {
		$sorted_query = $db_handle->runQuery("SELECT sewadar_name FROM sewadars WHERE short_name = '".$sr_short."' AND type = 'SR'");
		foreach ($sorted_query as $sorted_name) {
			array_push($sk_names, "SR: ".$sorted_name['sewadar_name']);
		}
	}
	...
	{		
		...
		for ($col=1; $col <= $col_count + 2; $col++) {
			...
			if ($row == 1) 
			{
				...
				if ($col == 1) {
					$table_str .= '<th>'.($row-1).'</th>';
				}
				else if ($col == 2) {
					$table_str .= '<td width = '.$col_width.' style="text-align: left;">'.explodeCheck($sk_names[$row - 2], ":").'</td>';
				}
				else {
