...
function get_table()
{
	...
	$pathi_type = array("bsp1", "bsp2", "bsp3", "bsp4", "bsp5");
	...
	foreach ($pathi_type as $p_type) {
		...
		if (!empty($sewa_list)) {
			foreach ($sewa_list as $sewa) {
				...
				$pathi_name = $sewa[$p_type];

				if (!in_array($pathi_name, $sk_names)) {
					array_push($sk_names, $pathi_name);
				}
			}
		}
	}
	...
	{		
		...
		if ($row != 1) {
			if ($sk_names[$row - 2] != '') {
				$sk_array = $db_handle->runQuery("SELECT * FROM `".$master_table."` WHERE `bsp1` = '".$sk_names[$row - 2]."' OR `bsp2` = '".$sk_names[$row - 2]."' OR `bsp3` = '".$sk_names[$row - 2]."' OR `bsp4` = '".$sk_names[$row - 2]."' OR `bsp5` = '".$sk_names[$row - 2]."'");
			}
			else {
				continue;
			}
