		...
		<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				...
				<?php
					...
					if (isset($_GET['id'])) {
						$sewadar_list = $db_handle->runQuery("SELECT * FROM `bsp` WHERE id = ".$_GET['id']);
				...
				?>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="text" name="mob" id="mob" class="col-xs-10 col-sm-5" value= "<?php echo $mob?>" />
						</div>
					</div>
...
<?php
	if(isset($_POST['Submit'])){
		...
		if(isset($_POST['sewadar_name'])&&!empty($_POST['sewadar_name'])){
			...
			$result = $db_handle->executeUpdate("UPDATE `bsp` set name = '".$_POST['sewadar_name']."', mob = '".$_POST['mob']."' WHERE  id=".$id);
		}
	}
