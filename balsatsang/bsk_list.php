...
function get_table()
{
	...
	$pathi_type = array("bsk", "extra_bsk");
	...
	{		
		...
		if ($row != 1) {
			if ($sk_names[$row - 2] != '') {
				$sk_array = $db_handle->runQuery("SELECT * FROM `".$master_table."` WHERE `bsk` = '".$sk_names[$row - 2]."' OR `extra_bsk` = '".$sk_names[$row - 2]."'");
			}
			else {
				continue;
			}
