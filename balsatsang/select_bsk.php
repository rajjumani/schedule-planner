...
include 'dbcontroller.php';
	...
	if (isset($_GET['centre']) && isset($_GET['date'])) {
		$centre_name = $_GET['centre'];
		$date = $_GET['date'];
		$row = $_GET['row'];
		$col = $_GET['col'];
		$master = $_GET['master'];
		$sewadar_type = $_GET['type'];
	}
	...
	<div class="col-xs-12">
		...
		<form class="form-horizontal" role="form" name="centreform" method="post">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="text" name="topic" id="topic" class="col-ms-2" autocomplete="off"/>
							</div>
						</div>
...
<?php
	...
	if(isset($_POST['sewadar_name'])&&!empty($_POST['sewadar_name'])){ 
		...
		if (strcmp($sewadar_type, "BSK") == 0) {
			$db = "bsk";
			$topic_type = "topic";
			$count = $db_handle->numRows("SELECT * FROM `".$master."` WHERE (`bsk` = '".$sewadar_name."' OR `extra_bsk` = '".$sewadar_name."') AND `date` = '".$date."'");
		}
		elseif (strcmp($sewadar_type, "Extra BSK") == 0) {
			$db ="extra_bsk";
			$topic_type = "extra_topic";
			$count = $db_handle->numRows("SELECT * FROM `".$master."` WHERE (`bsk` = '".$sewadar_name."' OR `extra_bsk` = '".$sewadar_name."') AND `date` = '".$date."'");
		}

		if ($count == 0) {
			
			$result = $db_handle->executeUpdate('UPDATE `'.$master.'` set '. $db . ' = "'.$sewadar_name.'", '.$topic_type.' = "'.$topic.'" WHERE centre_name = "'.$centre_name.'" AND date = "'.$date.'" AND col ='.$col);
