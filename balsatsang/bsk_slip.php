				...
				if (!in_array($pathi_name, $bsk_names)) {
					...
					for ($row = 1; $row <= 3; $row++) 
					{
						...
						for ($col=1; $col <= 4; $col++) {
							...
							{
								if ($col ==1) {
									$table_str .= '<th colspan = 3 width = "60%">'."BSK SEWA SLIP".'</th>';
									$col++;
								}
								elseif ($col ==3) {
							...
							elseif ($row == 3) 
							{
								...
								elseif ($col ==4) {
									$table_str .= '<th width = "35%">'."TOPIC".'</th>';
								}
							}
						}
					...
					$sr_array = $db_handle->runQuery("SELECT * FROM `".$master_table."` WHERE `bsk` = '".$pathi_name."' OR `extra_bsk` = '".$pathi_name."'");
					...
					foreach ($weekdays as $date => $day) {
						...
						foreach ($sr_array as $sr_name) {
							...
							if (strcmp($d, $date) == 0) {
								...
								if (strcmp($pathi_name, $sr_name['bsk']) == 0) {
									
									$sk_sr = $sr_name['topic'];
								} 

								elseif (strcmp($pathi_name, $sr_name['extra_bsk']) == 0) {
									
									$sk_sr = $sr_name['extra_topic'].'(Extra)';
								}  
							}
						}
					...
					$table_str .= '<th colspan = 4 height=20px style="text-align: right; padding-top: 15%;">'."AREA SECRETARY, AHMEDABAD".'</td>';
...
<div class="row">
	<div class="col-xs-12">
		...
		<?php echo get_table("bsk");
			  echo get_table("extra_bsk");	
