...
include 'dbcontroller.php';
	...
	if (strcmp($field_name, 'bsp1') == 0) {
		$sewadar_label = 'BSP 1';
	} 
	elseif (strcmp($field_name, 'bsp2') == 0) {
		$sewadar_label = 'BSP 2';
	} 
	elseif (strcmp($field_name, 'bsp3') == 0) {
		$sewadar_label = 'BSP 3';
	} 
	elseif (strcmp($field_name, 'bsp4') == 0) {
		$sewadar_label = 'BSP 4';
	}
	elseif (strcmp($field_name, 'bsp5') == 0) {
		$sewadar_label = 'Extra BSP';
	}
...
<div class="row">
	<div class="col-xs-12">
		...
		<form class="form-horizontal" role="form" name="centreform" method="post">
						...
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right" for="form-field-1"><?php echo $sewadar_label;?>: </label>
...
<?php
	...
	if(isset($_POST['sewadar_name'])&&!empty($_POST['sewadar_name'])){ 
		...
		$result = $db_handle->executeUpdate("UPDATE `".$master."` set ". $field_name . " = '".$sewadar_name."' WHERE centre_name = '".$centre_name."' AND date = '".$date."' AND col =".$col);
