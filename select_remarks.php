...
if(isset($_POST['sewadar_name'])&&!empty($_POST['sewadar_name'])){
	...
	if ($centre_name1 != "") {
		...
		$count = $db_handle->numRows("SELECT * FROM `".$master."` WHERE `row`= 50".$i." AND `col`= 50".$j);

		if ($count == 0) {

			$result = $db_handle->executeUpdate("INSERT INTO `".$master."` (`id`, `row`, `col`, `centre_name`, `date`, `day`, `sk_sr`, `pathi`, `ground`) VALUES ('', '50".$i."', '50".$j."', '".$centre_name1."', '".$date1."', '', '".$sewadar_type.": ".$sewadar_name."', '".$pathi1."', '".$ground1."')");
		}
		else {
			$result = $db_handle->executeUpdate("UPDATE `".$master."` set sk_sr = '".$sewadar_type.": ".$sewadar_name."', pathi = '".$pathi1."',  ground = '".$ground1."', centre_name = '".$centre_name1."', date = '".$date1."' WHERE row=50".$i." AND `col`= 50".$j);
		}
	...
	if ($centre_name2 != "") {
		...
		$count = $db_handle->numRows("SELECT * FROM `".$master."` WHERE `row`= 60".$i." AND `col`= 60".$j);

		if ($count == 0) {
			$result = $db_handle->executeUpdate("INSERT INTO `".$master."` (`id`, `row`, `col`, `centre_name`, `date`, `day`, `sk_sr`, `pathi`, `ground`) VALUES ('', '60".$i."', '60".$j."', '".$centre_name2."', '".$date2."', '', '".$sewadar_type.": ".$sewadar_name."', '".$pathi2."', '".$ground2."')");
		}
		else {
			$result = $db_handle->executeUpdate("UPDATE `".$master."` set sk_sr = '".$sewadar_type.": ".$sewadar_name."', pathi = '".$pathi2."',  ground = '".$ground2."', centre_name = '".$centre_name2."', date = '".$date2."' WHERE row=60".$i." AND `col`= 60".$j);
		}
	...
	if ($centre_name3 != "") {
		...
		$count = $db_handle->numRows("SELECT * FROM `".$master."` WHERE `row`= 70".$i." AND `col`= 70".$j);

		if ($count == 0) {
			$result = $db_handle->executeUpdate("INSERT INTO `".$master."` (`id`, `row`, `col`, `centre_name`, `date`, `day`, `sk_sr`, `pathi`, `ground`) VALUES ('', '70".$i."', '70".$j."', '".$centre_name3."', '".$date3."', '', '".$sewadar_type.": ".$sewadar_name."', '".$pathi."', '".$ground."')");
		}
		else {
			$result = $db_handle->executeUpdate("UPDATE `".$master."` set sk_sr = '".$sewadar_type.": ".$sewadar_name."', pathi = '".$pathi3."',  ground = '".$ground3."', centre_name = '".$centre_name3."', date = '".$date3."' WHERE row=70".$i." AND `col`= 70".$j);
		}
